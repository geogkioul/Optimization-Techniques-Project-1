function task_bisection_w_derivs(functions, a, b)
    % Inputs: 
    ...
    for i = 1:3 % For each function
        for j = 1:l_length % For every value of l
            l = l_vals(j);
            % Run bisection_w_derivs with that value
            [~, ~, f_vals] = bisection_w_derivs(functions{i}, a, b, l);
    ...
    for i=1:3 % For each function
        ...
        title(sprintf('Number of function %d evaluations for different l values - Bisection with derivatives method', i));
        legend('show');
        grid on;
    end
    ...
    for i = 1:3 % For each function
        ...
        for j = 1:l_length % For every value of l
            l = l_vals(j);
            % Run bisection_w_derivs with that value
            [a_vals, b_vals, ~] = bisection_w_derivs(functions{i}, a, b, l);
        ...
        title(sprintf('Values of interval borders [a,b] for function %d across iterations - Bisection with derivatives method', i));
